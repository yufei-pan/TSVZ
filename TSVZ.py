...
#: One-line 4.0 incompatibility notice. Used by the CLI and legacy wrappers;
#: ``import TSVZ`` itself is silent.
BREAKING_CHANGES_4_0 = (
	'TSVZ 4.0 follows tsvz-spec-v1 and is not compatible with TSVZ 3.x files. '
	'The 3.39 API remains as TSVZ_old.'
)
...
def apply_marker(state, f0_raw, value_fields, delimiter):
	...
	Unrecognized markers leave ``state`` unchanged. Advisory markers
	(``#_rotate_#``, ``#_write_ack_#``) are recorded on ``state`` but never
	affect reconstructed data (§12.4.3). Value fields are decoded without
...
def process_record(raw_line, state, store, delimiter, *, offset=None,
	...
	Args:
		...
		bound_states: Optional key→``(defaults, fill_empty, strip_trailing)``
			map recorded at each winning data row so an offset reader can
			re-materialize under §12.4.4 / §14.4, not the end-of-file state.
	...
	if bound_states is not None:
		bound_states[key] = (tuple(row_defaults), fill_empty, state.strip_trailing)
	return 'data', entry
...
def build_snapshot_preamble(state, delimiter):
	...
	if state.rotate != ROTATE_KEEP:
		lines.append(format_marker_line(MARKER_ROTATE, [state.rotate], delimiter))
	if state.defaults:
...
def read_offsets(path, *, create=False, encoding='utf8', delimiter=None,
	...
	Args:
		...
		bound_states: Optional mapping filled with the §14.4 marker snapshot
...
def snapshot_part(path, *, encoding='utf8', delimiter=None, header=None, store=None,
	...
	operates on what is already committed to disk: a key stored as ``<#>key``
	per §8.3 is real data and is preserved.
...
class _StoreCommon:
	...
	def _normalize_key(self, key):
		...
		key = str(key)
		if self._reader_state.strip_trailing:
			key = key.rstrip(' \t')
		return key
...
class WalStore(_StoreCommon, OrderedDict):
	...
	def __contains__(self, key):
		return OrderedDict.__contains__(self, self._normalize_key(key))
	...
	def _flush_locked(self):
		"""Drain and write. Caller must already hold ``self._lock``."""
...
class OffsetStore(_StoreCommon, MutableMapping):
	...
	*decoded* stream cannot address a compressed file, so seeking to a stored
	...
	def _row_binding(self, state=None):
		"""Freeze the §14.4 marker snapshot that applies to a new write."""
		state = self._reader_state if state is None else state
		return (tuple(state.defaults), state.fill_empty, state.strip_trailing)

	def _state_for_key(self, key):
		"""Reader state that applied at ``key``'s winning offset (§12.4.4)."""
		binding = None if key is None else self._bindings.get(key)
		if binding is None:
			return self._reader_state.copy()
		defaults, fill_empty, strip_trailing = binding
		st = ReaderState()
		st.defaults = list(defaults)
		st.fill_empty = fill_empty
		st.strip_trailing = strip_trailing
		st.return_on_missing = self._reader_state.return_on_missing
		return st
	...
	def flush(self, *, fsync=None):
		...
		if fsync:
			...
			# create=False on a missing path uses os.devnull; fsync is EINVAL there.
	...
	def _read_at(self, offset, key=None):
		...
		if line.endswith('\n'):  # §4.2: at most one \r immediately before the \n
			...
			if line.endswith('\r'):  # noqa: FURB188  # removesuffix needs 3.9+
				line = line[:-1]
		scratch = OrderedDict()
		kind, entry = process_record(
			line, self._state_for_key(key), scratch, self.delimiter)
		if kind == 'data' and entry is not None:
	...
	def __setitem__(self, key, value):
		...
		self._cache_put(key, list(row))
		self._bindings[key] = self._row_binding()
...
# ---------------------------------------------------------------------------
# Legacy API (pre-4.0 names and kwargs)
# ---------------------------------------------------------------------------
...
def __main__():
	...
	epilog = (
		...
		'  - a non-# first line is a data key, not a header\n'
		'  - a tombstone is a lone key line; key<TAB> is live empty cells, not a delete\n'
		'  - </sep/> and </LF/> are not decoded; # keys persist as <#>key\n'
		'  - a string defaults= argument is split character-by-character (pass a list)\n'
		'  - rewrite_on_load defaults to False\n'
		'  - -s/--strict means "missing file is an error" (not column checks); -v is gone\n'
	)
	parser = argparse.ArgumentParser(
		description='TSVZ 4.0: append-only tabular key–value store (tsvz-spec-v1)',
		epilog=epilog,
	...
	parser.add_argument(
		...
		help='Treat FILENAME as a §17 multi-part store. Numbered parts are '
			 'detected automatically; this flag is kept as an explicit alias.')
	parser.add_argument(
	...
	numbered = store_parts(args.filename)
	multipart = bool(numbered) or args.multipart
	if numbered:
		active_path = numbered[-1].path
		store_path = f'{numbered[0].store_path}.{numbered[0].format_ext}'
	else:
		active_path = args.filename
		store_path = args.filename
	...
	if args.operation == 'verify':
		paths = (store_part_paths(store_path) if multipart
				 else [args.filename])
		if not paths:
		...
		if digests.mismatches:
			for algo, want, got in digests.mismatches:
				print(f'tsvz: {algo} mismatch: expected {want}, computed {got}',
					  file=sys.stderr)
			return 1
		scope = 'store' if multipart else 'part'
		print(f'{digests.verified} segment(s) verified across {len(paths)} '
			  f'{scope} file(s); {len(digests.armed)} algorithm(s) armed')
		return 0
	if args.operation == 'read':
		try:
			reader = read_multipart if multipart else read_store
			data = reader(store_path, delimiter=args.delimiter)
		except FileNotFoundError:
			if args.strict:
		...
		append_record(
			active_path, args.line, create=True, header=header or None,
			delimiter=args.delimiter,
		)
		if args.checksum:
			append_checksum(active_path, args.checksum, delimiter=args.delimiter)
		...
		append_record(
			active_path, args.line[:1], create=True, header=header or None,
			delimiter=args.delimiter,
		)
	elif args.operation == 'clear':
		if numbered:
			live = read_multipart(store_path, delimiter=args.delimiter)
			if live:
...
if __name__ == '__main__':
	# ``python TSVZ.py --doctest`` or ``python -m doctest TSVZ.py``.
